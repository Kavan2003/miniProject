...
fun getLargestPaymentDetails(
    ...
    // Find largest payment for the product
    val paymentQuery = db.collection("user_payments")
        .whereEqualTo("product_reference", productId)
        .orderBy("pay_amt", Query.Direction.DESCENDING)
        .limit(1)
    ...
    paymentQuery.get().addOnCompleteListener { task ->
        if (task.isSuccessful) {
            ...
            if (documents.isNotEmpty()) {
                val paymentDoc = documents[0]
                val paymentAmount = paymentDoc.getString("pay_amt")
                val payingUserId = paymentDoc.getString("pay_by")

                // Find user details based on payingUserId
                val userQuery = db.collection("user_data").document(payingUserId!!)

                userQuery.get().addOnCompleteListener { innerTask ->
                    if (innerTask.isSuccessful) {
                        val userDoc = innerTask.result
                        a.address = userDoc.getString("Address")
            ...
            Log.w("TAG", "Error getting largest payment", task.exception)
            onUserDetailsFetched(null) // Call the callback with null for error
        }
    }
